...
lnorm = -0.5 * (np.log(2 * np.pi) * ndim_test +
...
if method == 'plot':
    ...
    if not os.path.exists(folderstring+f'/{filestring}_results'):
        ...
        try:
            ...
            sampler, previous_dlogz = getSampler(ndim_test, folderstring, filestring, loglikelihood=loglike, priortransform=ptform, parallel=parallel, sample=sample, resume_previous=resume_previous)
