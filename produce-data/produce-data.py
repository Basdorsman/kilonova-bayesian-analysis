...
dist = 40 #int(parameters['dist'])
read_data = 'kilonova' #parameters['read_data']
optical_delay = 12 #int(parameters['optical_delay']) # hours
delay = 0 #int(parameters['delay']) # hours
redden = False #parameters['redden']=='True'
# print('redden:',redden)
...
bp_f225w = stsyn.band('wfc3,uvis1,f225w')
bp_f336w = stsyn.band('wfc3,uvis1,f336w')
bs_uv = [bp_f225w, bp_f336w]
bs_uv_name = ['f225w','f336w']
...
for b, b_name in zip(bs_uv, bs_uv_name):#zip(bs_uv+bs_optical, bs_uv_name+bs_optical_name):
    if redden:
...
# SAVE DATA
print(f"saving data to ./input_files/data/SNR_fiducial_{read_data}_{dist}Mpc_uvbands_{bs_uv_string}_{delay}h_delay_redden_{redden}_optical_delay_{optical_delay}.pkl'")
import pickle
data = [t_data, abmags, snrs, AB_error, extinction_curves]
with open(f'../input_files/data/SNR_fiducial_{read_data}_{dist}Mpc_uvbands_{bs_uv_string}_{delay}h_delay_redden_{redden}_optical_delay_{optical_delay}.pkl', 'wb') as tf:
    pickle.dump(data,tf)
...
for b_name in bs_uv_name:
    ...
    ax.plot(t_UV_data.to_value('day'), abmags[b_name], label=b_name)
